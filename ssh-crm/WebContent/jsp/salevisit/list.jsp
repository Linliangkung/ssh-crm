							...
							<TBODY>
								<TR>
									<TD height=25>	
										...
										action="${pageContext.request.contextPath }/SaleVisitAction-list"
										method=post>
										<!-- 隐藏域.当前页码 -->
										<input type="hidden" name="currentPage" id="currentPageInput" value="<s:property value="#saleVisitPageBean.currentPage" />" />
										<!-- 隐藏域.每页显示条数 -->
										<input type="hidden" name="pageSize" id="pageSizeInput"       value="<s:property value="#saleVisitPageBean.pageSize" />" />
									...
									<TD>
										<TABLE id=grid
											...
											<TBODY>
												...
												<s:iterator value="#saleVisitPageBean.list" >
												<TR
													...
													<TD>
															<!-- 没有传递参数,显示删除和修改操作 -->
														<a href="${pageContext.request.contextPath }/SaleVisitAction-toEdit?visit_id=<s:property value="visit_id" />">修改</a>
														&nbsp;&nbsp;
								...
								<TR>
									<TD><SPAN id=pagelink>
											<DIV
												...
												共[<B><s:property value="#saleVisitPageBean.totalCount" /> </B>]条记录,[<B><s:property value="#saleVisitPageBean.totalPage" /></B>]页
												...
												<select name="pageSize" onchange="changePageSize($('#pageSizeSelect option:selected').val())" id="pageSizeSelect" >
													<option value="3" <s:if test="#saleVisitPageBean.pageSize==3">selected="selected"</s:if> >3</option>
													<option value="5" <s:if test="#saleVisitPageBean.pageSize==5">selected="selected"</s:if> >5</option>
												</select>
												条
												[<A href="javaScript:void(0)" onclick="changePage(<s:property value='#saleVisitPageBean.currentPage-1' />)" >前一页</A>]
												<B><s:property value="#pageBean.currentPage" /></B>
												[<A href="javaScript:void(0)" onclick="changePage(<s:property value='#saleVisitPageBean.currentPage+1' />)"  >后一页</A>] 
												到
												<input type="text" size="3" id="page" name="page" value="<s:property value="#saleVisitPageBean.currentPage" />"  />
												页
